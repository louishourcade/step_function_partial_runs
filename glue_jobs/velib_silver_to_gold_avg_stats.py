...
, month
, day
, avg(cast(numbikesavailable as int)) as avg_numbikesavailable
, avg(cast(mechanical as int)) as avg_mechanical
, avg(cast(ebike as int)) as avg_ebike
...
output_velib_avg_statistics = f"s3://{s3_bucket_name}/velib-avg-stats-gold"
S3bucket_Sink = glueContext.getSink(
        path=output_velib_avg_statistics,
        connection_type="s3",
...
S3bucket_Sink.setCatalogInfo(
    catalogDatabase=glue_database_name, catalogTableName=f'velib_avg_stats_gold'
)
